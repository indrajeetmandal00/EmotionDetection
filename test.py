...
while True:
    ...
    # 2. CNN Model Input PIP (Placeholder/Background)
    ...
    if len(faces) > 0:
        ...
        # Populate the CNN INPUT VIEW PIP
        # This shows exactly what the neural net is seeing (the 48x48 pixelated grayscale crop)
        # Using INTER_NEAREST to emphasize the raw pixel data fed to the model
        ...
        # Biometric BPM logic based on emotion
        if smooth_label in [0, 2]: # Angry, Fear
            target_bpm = 120 + random.randint(-5, 10)
        elif smooth_label == 6: # Surprise
            target_bpm = 100 + random.randint(-5, 5)
        elif smooth_label == 3: # Happy
            target_bpm = 85 + random.randint(-2, 5)
        elif smooth_label == 5: # Sad
    ...
    frame = cv2.addWeighted(graph_overlay, 0.6, frame, 0.4, 0)
    bio_color = (255, 255, 255) # White for biometrics default
    ...
    # Draw ECG style graph
    if len(bpm_history) > 1:
        # Scale bpm to graph height (assume max 150, min 60)
    ...
    inst_text = "[S] SNAPSHOT  |  [Q] QUIT"
    cv2.putText(frame, inst_text, (width//2 - 120, height - 15), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255), 1)

    # Handle Flash Effect
    if flash_frames > 0:
        flash_overlay = np.full((height, width, 3), 255, dtype=np.uint8)
        flash_alpha = flash_frames / 5.0 # Fade out over 5 frames
        frame = cv2.addWeighted(flash_overlay, flash_alpha, frame, 1 - flash_alpha, 0)
        flash_frames -= 1

    cv2.imshow("Emotion Analysis Studio",frame)
    ...
    if k==ord('q'):
        ...
        filename = f"captures/target_snap_{timestamp}.png"
        cv2.imwrite(filename, frame)
        print(f"Snapshot saved: {filename}")
        flash_frames = 5 # Trigger flash
